...
@implementation StreamingCameraManager

RCT_EXPORT_MODULE(StreamingCameraManager);
RCT_EXPORT_VIEW_PROPERTY(onCameraReady, RCTDirectEventBlock);
RCT_EXPORT_VIEW_PROPERTY(onMountError, RCTDirectEventBlock);
RCT_EXPORT_VIEW_PROPERTY(onStreaming, RCTDirectEventBlock);
...
- (UIView *)view
{
    return [[StreamingCamera alloc] initWithBridge:self.bridge];
}
...
RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, StreamingCamera)
{
...
RCT_CUSTOM_VIEW_PROPERTY(flashMode, NSInteger, StreamingCamera)
{
    [view setFlashMode:[RCTConvert NSInteger:json]];
    [view updateFlashMode];
}

RCT_CUSTOM_VIEW_PROPERTY(autoFocus, NSInteger, StreamingCamera)
{
    [view setAutoFocus:[RCTConvert NSInteger:json]];
    [view updateFocusMode];
}

RCT_CUSTOM_VIEW_PROPERTY(focusDepth, NSNumber, StreamingCamera)
{
    [view setFocusDepth:[RCTConvert float:json]];
    [view updateFocusDepth];
}

RCT_CUSTOM_VIEW_PROPERTY(zoom, NSNumber, StreamingCamera)
{
    [view setZoom:[RCTConvert CGFloat:json]];
    [view updateZoom];
}

RCT_CUSTOM_VIEW_PROPERTY(whiteBalance, NSInteger, StreamingCamera)
{
    [view setWhiteBalance:[RCTConvert NSInteger:json]];
    [view updateWhiteBalance];
}

RCT_CUSTOM_VIEW_PROPERTY(pictureSize, NSString *, StreamingCamera)
{
...
{
    [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, StreamingCamera *> *viewRegistry) {
        StreamingCamera *view = viewRegistry[reactTag];
        if (![view isKindOfClass:[StreamingCamera class]]) {
            RCTLogError(@"Invalid view returned from registry, expecting StreamingCamera, got: %@", view);
        } else {
